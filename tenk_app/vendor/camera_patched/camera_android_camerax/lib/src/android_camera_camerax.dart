...
class AndroidCameraCameraX extends CameraPlatform {
  ...
  Future<void> initializeCamera(
    ...
    // [tenk fork patch] VideoCapture 를 eager bind 로 전환, ImageAnalysis 를 lazy
    // 로 미룬다. 업스트림은 반대 — VideoCapture 가 lazy 라 startVideoCapturing 시점에
    // bindToLifecycle 이 다시 호출돼 Camera2 capture session 이 재구성되고
    // preview 가 잠깐 freeze 되는데, Tenk 의 2초 녹화 흐름에선 이 freeze 가 곧장
    // "버튼이 안 먹힌 것" 으로 인지된다. 우리는 image stream 을 사용하지 않으므로
    // ImageAnalysis 는 첫 호출 시(_configureImageAnalysis) lazy bind 해도 무해.
    // CameraX UseCase 조합 표 기준 Preview+ImageCapture+VideoCapture 는 LIMITED
    // 이상에서 지원 (4-way 는 LEVEL_3 한정이라 회피).
    camera = await processCameraProvider!.bindToLifecycle(
      cameraSelector!,
      <UseCase>[preview!, imageCapture!, videoCapture!],
    );
  ...
  Future<XFile> stopVideoRecording(int cameraId) async {
    ...
    // [tenk fork patch] VideoCapture 를 unbind 하지 않는다. 업스트림은 매 stop 마다
    // unbind 해서 다음 녹화 때 다시 bindToLifecycle 호출 → capture session 재구성
    // → preview freeze. 우리는 init 시점에 eager bind 했으므로 그대로 유지해 다음
    // 녹화도 즉시 시작되도록 한다.
